...
VALID_ACTIONS = np.arange(N_ACTION)
...
class Estimator():
    ...
    def _build_model(self):
        ...
        self.X_pl = tf.placeholder(shape=[None, 6], dtype=tf.float32, name="X")
        ...
        fc1 = tf.contrib.layers.fully_connected(self.X_pl, 256)
...
def deep_q_learning(sess,
    ...
    for i_episode in range(num_episodes):
        ...
        for t in itertools.count():
            ...
            states_batch.reshape(-1, states_batch.shape[-1])
            action_batch.reshape(-1, action_batch.shape[-1])
            reward_batch.reshape(-1, reward_batch.shape[-1])
            next_states_batch.reshape(-1, next_states_batch.shape[-1])
            done_batch.reshape(-1, done_batch.shape[-1])
...
def plot_reward(total_reward_all_eps):
    ...
    plt.plot(ma_vec, label="DQN for ATARI - Breakage")
    plt.xlabel('Episode')
    plt.ylabel('Moving-Average Rewards - window = 1000')
    plt.legend(loc='best')
    plt.show()
...
with tf.Session() as sess:
    ...
    for t, stats in deep_q_learning(sess,
                                    ...
                                    batch_size=32):
        # if t > 1000000 == 0:
        #     plot_reward(stats.episode_rewards)
        #
        # if t > 1500000 == 0:
        #     plot_reward(stats.episode_rewards)
        ep += 1
        print("Episode #{}, Frame #{}, Episode Reward: {}".format(ep, t, stats.episode_rewards[-1]))
