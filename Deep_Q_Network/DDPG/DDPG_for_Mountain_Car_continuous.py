...
class DDPG_Agent():
    def __init__(self):
        ...
        self.replay_buffer = []
        self.critic_estimator = Critic_Network(scope="estimator_critic")
        self.critic_target = Critic_Network(scope="target_critic")

        self.actor_estimator = Actor_Network(scope="estimator_actor")
        self.actor_target = Actor_Network(scope="target_actor")
    ...
    def update(self, sess, s, a, r, s_, done, learning = True, step_count=0):
        ...
        if learning:
            ...
            if step_count % UPDATE_AFTER_NUMBER_OF_INPUT == 0:
                self.great_update(sess, self.critic_estimator, self.critic_target)
                self.great_update(sess, self.actor_estimator, self.actor_target)
