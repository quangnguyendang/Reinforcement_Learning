...
class Actor_Network():
    ...
    def _model_build(self):
        with tf.variable_scope(self.scope):
            ...
            # fc1 = tf.contrib.layers.batch_norm(fc1, center=True, scale=True, is_training=self.isTraining)
            fc1 = tf.nn.relu(fc1, 'relu')
            fc2 = tf.contrib.layers.fully_connected(fc1, 300, activation_fn=None)
            # fc2 = tf.contrib.layers.batch_norm(fc2, center=True, scale=True, is_training=self.isTraining)
